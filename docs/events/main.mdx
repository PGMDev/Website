...
**Events** is the official PGM plugin for managing PvP matches in a competitive setting. 
...
The plugin also manages cycling and starting matches as well as readying teams. 
...
You will find more information about the plugin's commands in the `Commands` section. 
There is only one value to be configured, which is `observers-must-ready`. 
If disabled, the match will start without observers having to execute `/ready`. 
...
please check its [GitHub repository](https://github.com/PGMDev/Events).
:::
