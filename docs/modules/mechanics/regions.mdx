...
The area a region applies to is specified with one or more of the following elements. Use `oo` and `-oo` to specify positive/negative infinity in coordinates. See the [Properly Defining Regions](/docs/guides/xml-pointers/regions) guide for information on how to properly get the coordinates of a region.
