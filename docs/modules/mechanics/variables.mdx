...
Values are changed using the [Actions & Triggers](/docs/modules/gear/actions-triggers) mechanic.
...
Setting variables are done inside the [Actions & Triggers](/docs/modules/gear/actions-triggers) mechanic.
...
<actions>
    ...
    <trigger filter="flag-cap-filter" trigger="score-points" scope="player"/>
</action>
```
...
<actions>
    ...
    <trigger filter="region-a" trigger="score-points" scope="player"/>
    <!-- Sends a message to the team once the filter reached-score passes Allow -->
    <trigger filter="reached-score" trigger="completed" scope="team"/>
