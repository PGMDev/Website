...
A case branch describes the requirements for the text to be displayed. At least one of the `match` and `filter` properties must be specified. If both are specified, the filter acts as a "case guard". This means that both the range specified in `match` and the `filter` must match for the branch to match.
