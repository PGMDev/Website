...
The `proto=""` attribute specifies what PGM version a XML file was created for. If this value is higher than the version supported by the instance of PGM that is running, the map will not load. If it is lower, the map will load but the XML may be interpreted in an outdated way. In most cases, PGM will attempt to provide as much backward or forward compatibility as possible if a map is using a legacy proto version.
